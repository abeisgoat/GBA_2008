...
void MoveBall(void) {
   if(!(REG_KEYS & KEY_A)) {
      yball -= gspeed/2;
      gspeed -= 0.000000000000001;
}else{if (hit == 1) gspeed = 10;}
...
if (xball < xslope+6 && xball > xslope-6 && yball < yslope + 0 && yball > yslope - 8) {
     if ((xslope - xball) - 7 == -9) {yball = yslope + (xslope - xball) - 6;}else{yball = yslope + (xslope - xball) - 7;}
      hit = 1;
}else if (xball < xslope2+6 && xball > xslope2-6 && yball < yslope2 + 0 && yball > yslope2 - 8) {
     if ((xslope2 - xball) - 7 == -9) {yball = yslope2 + (xslope2 - xball) - 6;}else{yball = yslope2 + (xslope2 - xball) - 7;}
      hit = 1;
}
}
